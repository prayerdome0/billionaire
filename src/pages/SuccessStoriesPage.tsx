...
import { ArrowRight, Clock, Dices, Globe2, Play, Quote, RefreshCw, Sparkles, TrendingUp } from "lucide-react";
        ...
        <div className="flex flex-col gap-3 shrink-0">
          {video && (
            <button
              ...
              className="group inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-500 text-gray-900 font-bold px-6 py-3 text-sm transition-all hover:scale-105 shadow-lg shadow-amber-500/25"
            >
              <Play className="w-4 h-4 fill-current group-hover:scale-110 transition-transform" /> Watch {current.name.split(" ")[0]}'s Video
            </button>
          )}
          <button
            onClick={roll}
            className="group inline-flex items-center gap-2 rounded-xl border border-amber-500/40 bg-amber-500/10 hover:bg-amber-500/20 px-6 py-3 text-sm font-bold text-amber-300 transition-all"
          >
...
/** Video library — every successful person's photo now has a real video of them. */
        ...
        <Reveal className="flex items-center gap-3 mb-3">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-white">
            ...
            <span className="bg-gradient-to-r from-amber-300 to-yellow-500 bg-clip-text text-transparent">
              a video after every photo
            </span>
          </h2>
        </Reveal>
        <Reveal>
          <p className="text-gray-400 text-sm leading-relaxed mb-8 max-w-3xl">
            Every successful person above now has their real story on video. Each one opens with{" "}
            <span className="text-amber-300 font-semibold">“Welcome to Seedwel Investment Limited, here is…”</span>{" "}
            and ends with{" "}
            <span className="text-amber-300 font-semibold">“Thank you for watching.”</span> Tap any card to watch.
          </p>
        </Reveal>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {withVideo.map((story, i) => {
            const v = storyToVideo(story);
            if (!v) return null;
            return (
              <Reveal key={story.id} direction="up" delay={(i % 3) * 80}>
                <button
                  onClick={() => setPlaying(v)}
                  className="group text-left w-full bg-gray-900/60 border border-gray-800 rounded-2xl overflow-hidden hover:border-amber-500/40 hover:-translate-y-1 hover:shadow-xl hover:shadow-amber-500/5 transition-all duration-300"
                >
                  <div className="relative aspect-video overflow-hidden">
                    <img
                      src={videoThumbnail(v.youtubeId)}
                      alt={`${story.name} — ${v.title}`}
                      loading="lazy"
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-gray-950/90 via-gray-950/20 to-transparent" />
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-14 h-14 rounded-full bg-amber-500/95 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg shadow-amber-500/30">
                        <Play className="w-6 h-6 text-gray-900 fill-current ml-0.5" />
                      </div>
                    </div>
                    <span className="absolute top-3 right-3 flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full bg-black/70 text-amber-300 border border-amber-500/30">
                      <Clock className="w-3 h-3" /> {v.duration}
                    </span>
                    <span className="absolute bottom-3 left-3 text-[10px] font-bold px-2.5 py-1 rounded-full bg-black/70 text-emerald-300 border border-emerald-500/30">
                      {story.netWorth}
                    </span>
                  </div>
                  <div className="p-5">
                    <p className="text-[10px] uppercase tracking-widest text-amber-400/80 font-semibold mb-1.5">
                      {story.country} • {story.category}
                    </p>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <PageHeader
        ...
        description="Photos, quotes and words of motivation from the world's most successful founders, investors and change-makers — including Africa's own Aliko Dangote, Strive Masiyiwa, Tony Elumelu and Mo Abudu. Every photo now comes with a real video of that person, opening with a Seedwel welcome and ending with a thank you. If they could rise from where they started, so can you."
      />
      ...
      {/* NEW — video library: a video after every photo */}
      <StoryVideoLibrary />
        ...
        <div className="max-w-6xl mx-auto px-4">
          <Reveal className="rounded-3xl bg-gray-900/40 border border-gray-800 p-8 md:p-10">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div>
                <h3 className="text-lg md:text-xl font-black text-white">Enjoying the videos & stories?</h3>
                <p className="text-gray-500 text-sm mt-1">Rate this page — it helps Seedwel make better content.</p>
              </div>
