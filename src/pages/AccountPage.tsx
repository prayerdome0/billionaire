...
export default function AccountPage() {
  ...
  const isIssuedCert = isCertificateIssued(cert);
  const isAwaitingCertificate = isCertificateAwaitingAdmin(cert);
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <PageHeader
        ...
        description={`Your Seedwel student dashboard — tuition FREE. Send a $${CERTIFICATE_FEE} payment quotation to the admin; after verification, the certificate is sent within ${CERTIFICATE_DELIVERY_WINDOW_HOURS} hours. Powered by Firebase Firestore.`}
      />
      ...
      <section className="max-w-6xl mx-auto px-4 pb-20 space-y-8">
        ...
        <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-br from-amber-500/5 to-gray-900/60 p-5 flex flex-col md:flex-row gap-4 md:items-center justify-between">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1 max-w-2xl">
                Tuition is <span className="text-emerald-300 font-bold">$0 FREE</span> worldwide. Send a {" "}
                <span className="text-amber-300 font-bold">${CERTIFICATE_FEE} USD</span> payment quotation to the admin; after verification, your certificate is sent within ${CERTIFICATE_DELIVERY_WINDOW_HOURS} hours.
                True database: <code className="text-amber-400">Firebase Firestore • project: seedwel-cbeb8</code>
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span className="rounded-full bg-amber-500/15 border border-amber-500/30 px-3 py-1 text-[11px] font-bold text-amber-300 flex items-center gap-1">
              <DollarSign className="w-3 h-3" /> Cert quotation ${CERTIFICATE_FEE}
            </span>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="rounded-3xl bg-gray-900/70 border border-gray-800 p-6">
            ...
            {/* Certificate request / delivery status */}
            {cert && (
              <div className="mt-4 rounded-xl border border-gray-800 bg-gray-950/60 p-3 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`font-bold ${isIssuedCert ? "text-emerald-300" : "text-amber-300"}`}>
                    {isIssuedCert ? "Sent ✓" : isAwaitingCertificate ? `Admin queue • ${CERTIFICATE_DELIVERY_WINDOW_HOURS}h` : `Send $${CERTIFICATE_FEE} quotation`}
                  </span>
                </div>
            ...
            {nextLesson ? (
              ...
              <Link
                ...
                className={`inline-flex items-center justify-center gap-2 rounded-xl font-bold px-4 py-2.5 text-xs ${
                  isIssuedCert ? "bg-gradient-to-r from-emerald-500 to-teal-500 text-white" : "bg-gradient-to-r from-amber-500 to-yellow-500 text-gray-950"
                }`}
              >
                <Award className="w-4 h-4" /> {isIssuedCert ? "View Issued Certificate" : isAwaitingCertificate ? "Certificate Request in Review" : `Send Certificate Quotation $${CERTIFICATE_FEE}`}
              </Link>
            )}
          </div>
        ...
        <div className="grid sm:grid-cols-4 gap-4">
          ...
          <Link to="/certificate" className="rounded-2xl bg-gray-900/70 border border-gray-800 p-5 hover:border-amber-500/40 transition-all group">
            ...
            <div className="font-bold text-sm group-hover:text-amber-300">Certificate — ${CERTIFICATE_FEE} Quotation</div>
            <div className="text-xs text-gray-500 mt-1">
              {isIssuedCert ? "Sent ✓ View issued copy" : isAwaitingCertificate ? `Admin sends within ${CERTIFICATE_DELIVERY_WINDOW_HOURS} hours` : `Send $${CERTIFICATE_FEE} quotation to admin`}
            </div>
          </Link>
